...
export class InvoicesPage implements OnInit {
  ...
  openEditModal(invoice: any) {
    ...
    this.api.getInvoiceDetails(invoice.id).subscribe({
      next: (res: any) => {
        this.isLoading = false;
        // Merge list-level customer info so it's always available
  ...
  openCheckPreview() {
    ...
    this.api.previewInvoice(this.selectedInvoice.id).subscribe({
      ...
      error: () => {
        // Fallback: show preview with existing data
  ...
  downloadReceipt() {
    ...
    // Use canvas approach for download
    const printWindow = window.open('', '_blank');
    if (!printWindow) {
      this.showToastMsg('Please allow pop-ups to download');
      return;
    }
    ...
    const styles = `
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Courier New', monospace; background: #F0EBE3; display: flex; justify-content: center; padding: 40px 20px; }
        .receipt { background: #fff; border-radius: 24px; padding: 40px 36px; max-width: 480px; width: 100%; box-shadow: 0 4px 24px rgba(0,0,0,0.08); }
        .receipt-type { display: block; text-align: center; font-size: 13px; letter-spacing: 6px; color: #888; margin-bottom: 16px; }
        .divider { height: 1px; background: #1a1a1a; margin: 12px 0; }
        .divider-thin { height: 1px; background: #ddd; margin: 12px 0; }
        .company { text-align: center; font-size: 22px; font-weight: 700; margin: 12px 0 4px; }
        .co-reg { display: block; text-align: center; font-size: 12px; color: #888; margin-bottom: 8px; }
        .address { display: block; text-align: center; font-size: 11px; color: #666; line-height: 1.6; }
        .contact { display: block; text-align: center; font-size: 11px; color: #888; margin-top: 6px; }
        .doc-row { display: flex; gap: 12px; margin: 4px 0; }
        .doc-label { font-size: 12px; font-weight: 700; min-width: 70px; }
        .doc-value { font-size: 12px; font-weight: 700; }
        .to-section { margin: 16px 0; }
        .to-label { font-size: 12px; font-style: italic; color: #888; }
        .to-box { border: 1px solid #ddd; border-radius: 8px; padding: 12px; margin-top: 6px; font-size: 12px; line-height: 1.6; }
        .table-header { display: flex; justify-content: space-between; font-size: 11px; font-weight: 700; font-style: italic; }
        .item-row { margin: 12px 0; }
        .item-desc { display: flex; justify-content: space-between; font-size: 12px; font-weight: 700; }
        .item-calc { font-size: 11px; color: #888; margin-top: 2px; display: flex; justify-content: space-between; }
        .total-row { display: flex; justify-content: space-between; font-size: 12px; margin: 4px 0; }
        .net-bar { background: #1a1a1a; color: #fff; border-radius: 8px; padding: 14px 20px; display: flex; justify-content: space-between; align-items: center; margin: 16px 0; }
        .net-label { font-size: 12px; font-weight: 700; font-style: italic; }
        .net-value { font-size: 20px; font-weight: 700; }
        .due-box { border: 1px solid #ddd; border-radius: 8px; padding: 16px; text-align: center; margin: 16px 0; }
        .due-label { display: block; font-size: 10px; letter-spacing: 3px; color: #888; margin-bottom: 6px; }
        .due-date { font-size: 18px; font-weight: 700; }
        .sig-box { border: 1px solid #ddd; border-radius: 8px; padding: 16px; min-height: 100px; margin: 16px 0; }
        .sig-label { font-size: 11px; color: #ccc; font-style: italic; }
        .thanks { text-align: center; font-size: 12px; letter-spacing: 6px; color: #ccc; margin-top: 20px; }
        .cn-header { font-size: 11px; font-weight: 700; letter-spacing: 2px; color: #1a1a1a; margin: 8px 0; }
        .cn-row { display: flex; justify-content: space-between; padding: 4px 0; font-size: 12px; }
        .cn-number { font-weight: 700; }
        .cn-reason { font-size: 10px; color: #888; }
        .cn-amount { font-weight: 700; color: #1a1a1a; }
        .cn-deduct { color: #1a1a1a; font-weight: 700; }
      </style>
    `;
    ...
    items.forEach((item: any, i: number) => {
      ...
      itemsHtml += `
        <div class="item-row">
          <div class="item-desc"><span>${i + 1}. ${prodName} (${item.uom || 'UNIT'})</span><span>[${item.taxType || 'SR'}]</span></div>
          <div class="item-calc"><span>${item.quantity} × ${(item.unitPrice || 0).toFixed(2)}</span><span>${subtotal}</span></div>
        </div>
      `;
    });
    ...
    printWindow.document.write(`
      <!DOCTYPE html><html><head><title>Invoice ${inv?.invoiceNumber || ''}</title>${styles}</head>
      <body>
        <div class="receipt">
          ...
          <div style="margin-top:20px;">
            <div class="doc-row"><span class="doc-label">DOC NO</span><span class="doc-value">: ${inv?.invoiceNumber || 'S001-' + inv?.id}</span></div>
            <div class="doc-row"><span class="doc-label">DATE</span><span class="doc-value">: ${dateStr}</span></div>
          </div>

          <div class="to-section">
            <span class="to-label">TO:</span>
            <div class="to-box"><strong>${inv?.customerName || this.getCustomerName(inv?.customerId)}</strong>${(() => { const c = this.getCustomer(inv?.customerId); return c?.address ? '<br>' + c.address : ''; })()}</div>
          </div>

          <div class="divider-thin"></div>
          <div class="table-header"><span>DESCRIPTION</span><span>GST &nbsp;&nbsp; SUBTOTAL</span></div>
          <div class="divider-thin"></div>
          ${itemsHtml}
          <div class="divider-thin"></div>
          <div class="total-row"><span>GROSS TOTAL</span><span>RM ${(inv?.totalAmount || 0).toFixed(2)}</span></div>
          <div class="total-row"><span>TAX TOTAL</span><span>RM ${pd?.taxTotal || '0.00'}</span></div>
          ${(() => {
            const cns = inv?.creditNotes || [];
            if (cns.length === 0) return '';
            const totalCN = cns.reduce((s: number, c: any) => s + (c.amount || 0), 0);
            let cnHtml = '<div class="divider-thin"></div><div class="cn-header">CREDIT NOTE(S)</div>';
            cns.forEach((cn: any) => {
              cnHtml += `<div class="cn-row"><span class="cn-number">${cn.cnNumber || 'CN-' + cn.id}${cn.reason ? ' <span class="cn-reason">(' + cn.reason + ')</span>' : ''}</span><span class="cn-amount">- RM ${(cn.amount || 0).toFixed(2)}</span></div>`;
            });
            cnHtml += '<div class="divider-thin"></div>';
            cnHtml += `<div class="total-row"><span>SUBTOTAL</span><span>RM ${(inv?.totalAmount || 0).toFixed(2)}</span></div>`;
            cnHtml += `<div class="total-row cn-deduct"><span>CREDIT NOTE</span><span>- RM ${totalCN.toFixed(2)}</span></div>`;
            return cnHtml;
          })()}

          <div class="net-bar"><span class="net-label">NET AMOUNT</span><span class="net-value">RM ${((inv?.totalAmount || 0) - this.getTotalCN()).toFixed(2)}</span></div>

          <div class="due-box"><span class="due-label">PAYMENT DUE</span><span class="due-date">${dueStr}</span></div>
          <div class="sig-box"><span class="sig-label">SIGNATURE</span></div>
